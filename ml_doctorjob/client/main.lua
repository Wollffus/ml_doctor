...
RegisterCommand('doctor_help', function() -- client

    
        print("sent help")
	    local coords = GetEntityCoords(PlayerPedId())
        
        TriggerServerEvent("ml_doctorjob:alert_me", coords)
...
RegisterNetEvent("ml_doctorjob:GetAlertNow") -- client
AddEventHandler("ml_doctorjob:GetAlertNow", function(coords)

    TriggerEvent("redemrp_notification:start", "Someone needs your help! Open your map", 2)

    local blip = N_0x554d9d53f696d002(1664425300, coords)
    SetBlipSprite(blip, -1918326968, 1)
    Citizen.InvokeNative(0x9CB1A1623062F402, blip, "Injured Player") 
   
    Wait(Config.DeadBlipTime)
    RemoveBlip(blip)
end)

RegisterNetEvent("ml_doctorjob:check_client") -- client
AddEventHandler("ml_doctorjob:check_client", function(coords)
    

    TriggerServerEvent("ml_doctorjob:check_job", coords)
end)
